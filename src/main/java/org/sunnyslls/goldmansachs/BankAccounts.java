...
public class BankAccounts {
    ...
    static String feeOrUpfront(int n, int k, int x, int d, int[] p) {
        double totalSalaryPaymentDeduction = 0;
        for(int payment : p){
            double bankCharge = (Double.valueOf(x) / Double.valueOf(100)) * payment;
            totalSalaryPaymentDeduction += Math.max(k, bankCharge);
        }
        String result = "fee";
        if(totalSalaryPaymentDeduction > d){
            result = "upfront";
        }
        return result;
    }
